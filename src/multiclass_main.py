...
for dataset in ["google"]:
    for training_framework in ["multiclass"]:
        for model_name in ["gravity_gae", "sourcetarget_gae","mlp_gae", "digae","magnet"]:
            ...
            #res = methods.evaluate_link_prediction(model,  test_data_general, metrics_dict, test_data_hitsk_mrr= test_data_general_hitsk_mrr, device = device)
            ...
            preds_directional = []
            preds_bidirectional = []
            # general_metrics_dict = {"mrr": methods._eval_mrr}
            metrics_dict = {"aucroc":  methods.aucroc, "ap": methods.average_precision}

            for i in range(num_runs):

                train_data_general, train_data_directional, train_data_bidirectional, val_data_general, val_data_directional, val_data_bidirectional, test_data_general, test_data_directional, test_data_bidirectional, test_data_general_hitsk_mrr = (None, None, None, None, None, None, None, None, None,None)
                torch.cuda.empty_cache()

                train_data_general, train_data_directional, train_data_bidirectional, val_data_general, val_data_directional, val_data_bidirectional, test_data_general, test_data_directional, test_data_bidirectional, test_data_general_hitsk_mrr = methods.get_split_3_tasks_scipy(dataset, features_type,  add_remaining_self_loops_supervision, use_sparse_representation, True, device)
                ...
                preds_general.append(methods.evaluate_link_prediction(model, test_data_general, metrics_dict = metrics_dict, test_data_hitsk_mrr=test_data_general_hitsk_mrr ,device = device))
                # preds_general_hitsk_mrr.append(methods.evaluate_link_prediction(model, test_data_general_hitsk_mrr, metrics_dict = metrics_dict, device = device))
            ...
            df = pd.DataFrame(data = np.array([[pred["aucroc"] for pred in preds_general], [pred["ap"] for pred in preds_general], [pred["hits@1"] for pred in preds_general], [pred["hits@3"] for pred in preds_general], [pred["hits@10"] for pred in preds_general], [pred["mrr"] for pred in preds_general],
                                        [pred["aucroc"] for pred in preds_directional], [pred["ap"] for pred in preds_directional],
                                        [pred["aucroc"] for pred in preds_bidirectional], [pred["ap"] for pred in preds_bidirectional]]).T, columns = ["G_ROC_AUC", "G_AUPRC", "G_H1", "G_H3", "G_H10", "G_MRR", "D_ROC_AUC","D_AUPRC","B_ROC_AUC","B_AUPRC"])
